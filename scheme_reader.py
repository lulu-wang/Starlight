...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 7B
        return Pair('quote', Pair(scheme_read(src), nil)) #make into quote format, make recursive call within pair
        # END PROBLEM 7B
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            src.remove_front() #remove parenthesis
            return nil #end of pair
            # END PROBLEM 1
        elif src.current() == '.':
            # BEGIN PROBLEM 2
            src.remove_front() #get rid of dot
            formatted_src = scheme_read(src) #format src into pair
            if src.current()!=')': #check if there is more than one element after dot.
                raise SyntaxError ('Cannot have more than one element after dot.')
            else:
                src.remove_front() #get rid of parenthesis
                return formatted_src
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
            read_pair = Pair (scheme_read(src), read_tail(src)) #make src into pair format
            return read_pair
