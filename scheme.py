...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        try:
            ...
            return operator.eval_call(rest,env) #apply operator on operands using eval_call
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        ...
        scheme_eval(expressions.first, env) #evaluate first part of expressions.
        return eval_all(expressions.second, env) #recursive call to rest of expressions.
    else:
        return scheme_eval(expressions.first,env, True) #tail recursive, so apply scheme_optimized_eval
        #if rest of expressions is nil, evaluate only first part of expressions.
    # END PROBLEM 8
...
class Frame:
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 3
        self.bindings[symbol] = value #bind symbol to value in bindings dictionary
        # END PROBLEM 3
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            return self.bindings[symbol] #return value if symbol exists in current frame's bindings
        elif self.parent != None:
            return self.parent.lookup(symbol) #look up symbol in bindings of parent frame if it exists
        # END PROBLEM 3
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        if len(formals) == len(vals): #check if formals and values have the same number of arguments; if not, raise an error.
            while formals!= nil and vals!= nil:
                #loops through to assign formal to value until each list is exhausted.
                formal = formals.first
                value = vals.first
                child.bindings[formal] = value
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        #apply operator self onto each operand by mapping scheme_eval on operands arguments.
        # END PROBLEM 5
    ...
    def apply(self, args, env):
        ...
        try:
            if self.use_env == True:
                python_args.append(env) #add current environment env as last argument to python_args list
            return self.fn(*python_args) #call self.fn on all arguments in python_args
        except TypeError:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        #binds target to expression.
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        name = target.first #establish name of procedure
        target_params = expressions.first.second #params of procedure
        procedure = expressions.second #what needs to be evaluated
        param_exp_pair = Pair (target_params, procedure) #pair parameters with procedure
        env.define(name, do_lambda_form(param_exp_pair, env)) #bind name of procedure to parameters and procedure
        return name
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    #creates LambdaProcedure instance using formals and expressions
    # END PROBLEM 9
...
def do_if_form(expressions, env):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)):
        ...
        return scheme_eval(expressions.second.second.first, env, True)#tail recursive, so apply scheme_optimized_eval

def do_and_form(expressions, env):
    """Evaluate a (short-circuited) and form."""
    # BEGIN PROBLEM 13
    check_cond = True #returns true if iterative cases all pass without returning anything (an empty expression returns true).
    while expressions!= nil: #iteratively check each expression in expressions to return either true, false, or evaluated value.
        check_cond = scheme_eval(expressions.first, env, expressions.second==nil) #re-assign check_cond to evaluated expression
        if scheme_falsep(check_cond): #if expression equates to false python value, have to return false.
            return check_cond
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    if expressions is nil: #an empty expression evaluates to false.
        return False
    if expressions.first is True: #return first true value
        return True
    if scheme_truep(scheme_eval(expressions.first, env, expressions.second == nil)): #evaluate the python truth value of expression while checking if the last case, expressions.second==nil, is true.
        #if expression isn't false value or the term 'true', evaluate and return expression.
        return scheme_eval(expressions.first, env, expressions.second == nil)
    return do_or_form(expressions.second, env) #recursively check rest of expressions
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            #evaluate and return clause.first if rest of clause is empty. Otherwise, return evaluated rest of clause.
            if result is None:
                return test
            return result
            # END PROBLEM 14
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    names, values = nil, nil #start with nil values to pair.
    while bindings != nil:
        #iteratively run through bindings while checking structure of each binding and accumulating two lists, names and values respectively.
        to_bind = bindings.first
        check_form(to_bind, 2, 2)
        names = Pair(to_bind.first, names) #recursively accumulate a list of names in bindings
        values = Pair(scheme_eval(to_bind.second.first, env), values) #recursively accumulate a list of values in bindings
        bindings = bindings.second
    check_formals(names) #check that formal parameters in names are scheme list of symbols
    return env.make_child_frame(names, values) #bind names to values in a child frame, return that child.
    # END PROBLEM 15
...
class MuProcedure(UserDefinedProcedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self,args,env):
        return env.make_child_frame(self.formals,args) #dynamic scoping using environment from which the MuProcedure is called, by making a child frame containing current frame arguments
    # END PROBLEM 16
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    return MuProcedure(formals,expressions.second) #return MuProcedure instance with dynamic scoping
    # END PROBLEM 16
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            operator = scheme_eval(first, env) #evaluate operator
            check_procedure(operator) #check to see if operator is correct procedure
            args = rest.map(lambda x: scheme_eval(x,env)) #evaluate every value within expr.second (expressions to be calculated); accumulate these.
            result = scheme_apply(operator, args, env) #apply operator to arguments within environment; effectively evaluating all expressions.
            # END Extra Credit
    return result
